...
public class SelEffectBridge extends BaseEffectBridge {
	...
	public void onFocusView(View focusView, float scaleX, float scaleY) {
		...
		if (focusView != null) {
			runTranslateAnimation(focusView);
		}
	}
	...
	public void flyWhiteBorder(final View focusView, View moveView) {
		int newWidth = 0;
		int newHeight = 0;
		int oldWidth = 0;
		int oldHeight = 0;
		...
		if (focusView != null) {
			// 有一点偏差,需要进行四舍五入.
			newWidth = (int) Math.rint(focusView.getMeasuredWidth());
			newHeight = (int) Math.rint(focusView.getMeasuredHeight());
...
//					getMainUpView().requestLayout();
	...
	public void runTranslateAnimation(View toView) {
		flyWhiteBorder(toView, getMainUpView());
	}
